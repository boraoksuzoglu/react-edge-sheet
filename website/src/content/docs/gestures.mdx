...
```tsx
<Sheet ref={ref} edge="bottom" showDragHandle snapPoints={['200px', '50vh', '90vh']}>
  <div style={{ padding: '2rem' }}>
...
>
  <div style={{ padding: '1rem 1.5rem' }}>{/* mini player or full player depending on snap */}</div>
</Sheet>
```
...
>
  <nav style={{ padding: '2rem' }}>{/* navigation items */}</nav>
</Sheet>
```
