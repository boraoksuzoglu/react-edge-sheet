...
When the sheet is open, the body scroll is locked. By default, `padding-right` equal to the scrollbar width is applied to avoid layout shift. Control this with `scrollLockPadding`:

```tsx
<Sheet scrollLockPadding={false}>...</Sheet>   // no padding (accept layout shift)
<Sheet scrollLockPadding="1rem">...</Sheet>    // custom value
```
...
When the sheet is open, body scroll is locked. By default, `padding-right` is set to the scrollbar width to prevent layout shift. Control this via `scrollLockPadding`:

```tsx
// Disable padding (may cause layout shift when scrollbar hides)
