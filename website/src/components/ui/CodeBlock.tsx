...
export function CodeBlock({ html, code, filename, className }: CodeBlockProps) {
  return (
    <div
      className={`relative rounded-xl overflow-hidden border border-[color-mix(in_oklch,var(--glass-border)_40%,transparent)] bg-[color-mix(in_oklch,var(--glass-surface)_70%,transparent)] ${className ?? ''}`}
    >
      {filename && (
