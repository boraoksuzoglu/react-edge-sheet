...
export function RightContent({ onClose }: { onClose: () => void }) {
  ...
  const row = (title: string, sub: string, on: boolean, toggle: () => void) => (
    <div
      style={{
        display: 'flex',
        alignItems: 'center',
  ...
  return (
    <div style={{ padding: '1.5rem' }}>
      ...
      {row('Notifications', 'Push and in-app alerts', notifs, () => setNotifs((v) => !v))}
      <Divider />
      {row('Sound effects', 'Play sounds on interactions', sounds, () => setSounds((v) => !v))}
      <Divider />
      {row('Animations', 'Smooth transitions and motion', animations, () =>
        setAnimations((v) => !v)
      )}
