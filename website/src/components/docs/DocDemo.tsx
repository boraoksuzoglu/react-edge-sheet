...
function DefaultContent({ side, onClose }: { side: SheetEdge; onClose: () => void }) {
  return (
    <div>
      ...
      >
        Slides in from the <strong style={{ color: 'var(--foreground)' }}>{side}</strong> edge.
        Press{' '}
        <kbd
          style={{
            fontSize: '0.75rem',
...
export function DocDemo({
  ...
  const isDynamic =
    label?.toLowerCase().includes('dynamic') || label?.toLowerCase().includes('height');
    ...
    <div style={{ margin: '1.25rem 0' }}>
      <button
        ...
        onMouseEnter={(e) => {
          ...
          e.currentTarget.style.boxShadow = '0 8px 20px -4px oklch(68% 0.22 290 / 0.4)';
        }}
        onMouseLeave={(e) => {
          e.currentTarget.style.transform = '';
          e.currentTarget.style.boxShadow = '0 4px 12px -2px oklch(68% 0.22 290 / 0.3)';
        }}
      >
        <svg
          width="14"
